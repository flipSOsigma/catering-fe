...
export default function TestingPage() {
  // Initial state without auth dependency
  const [order, setOrder] = useState<OrderData>({
    event_name: '',
    invitation: 0,
    visitor: 0,
    created_by: 'test-user', // Removed auth dependency
    created_at: new Date(),
    note: '',
    price: 0,
    portion: 0,
  ...
  // Validation logic
  const validateOrder = useCallback((order: OrderData): { isValid: boolean; message: string } => {
    if (
      !order.event_name ||
  ...
  const addPortion = useCallback((sectionId: string) => {
    setOrder(prev => {
      ...
      const updatedSections = prev.sections.map(s => 
        s.id === sectionId 
          ? { ...s, portions: [...s.portions, newPortion] } 
          : s
      );

      const updatedOrder = { ...prev, sections: updatedSections };
      setValidation(validateOrder(updatedOrder));
      return updatedOrder;
    });
  }, [createEmptyPortion, validateOrder]);
